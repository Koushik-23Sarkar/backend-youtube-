...
const updateAccountDetails = asyncHandler(async (req, res) => {
  ...
  const user = await User.findByIdAndUpdate(
    ...
    { new: true } //By using this line, we will get information after Update.
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  const channel = await User.aggregate([
    ...
    {
      $lookup: {
        //use for join method (basically to join two tables)
    ...
    {
      $addFields: {
        // to add new fields in the table
        subscribersCount: {
    ...
    {
      $project: {
        // you may have so many field, $project will return only the selected field.
        fullName: 1,
        username: 1,
        subscribersCount: 1,
  ...
  const user = await User.aggregate([
    {
      $match: {
        ...
        _id: new mongoose.Types.ObjectId(req.user._id), // convert that string into "MongoDb.ObjectId"
      }, // In find method, we don't need to convert it into "MongoDb.ObjectId". find Method automatically handle that but, $match method doesn't .
    },
    {
      $lookup: {
        from: "videos",
        ...
        as: "watchHistory",
        pipeline: [
          //nested aggregation  //sub pipelines
          {
            // Because we need "Ower".
            $lookup: {
              from: "users",
              localField: "owner",
              foreignField: "_id",
              as: "owner",
              pipeline: [
                // Because we don't need all the information of Ower field
                {
                  $project: {
                    fullName: 1,
                    username: 1,
