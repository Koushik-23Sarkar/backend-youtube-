...
const getUserChannelProfile = asyncHandler(async(req, res) => {
    ...
    const channel = await User.aggregate([  //After aggregate pipeline returns Array.
        {
            $match: {   //find method   
                username: username?.toLowerCase()
    ...
    const user = await User.aggregate([
        ...
        },
        {
            $lookup: {
                ...
                foreignField: "_id",
                as: "watchHistory",
                pipeline: [   //nested aggregation  //sub pipelines  
                    {       // Because we need "Ower".
                        $lookup: {
                            ...
                            foreignField: "_id",
                            as: "owner",
                            pipeline: [     // Because we don't need all the information of Ower field
                                {
                                    $project: {
                                        fullName: 1,
                                        username: 1,
